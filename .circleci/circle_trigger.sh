...
for PACKAGE in ${LIBS_PACKAGES[@]}
do
  PACKAGE_PATH=${LIBS_ROOT#.}/$PACKAGE
...
echo "Changes detected in ${COUNT} libs/ project(s)."
...
do
  PACKAGE_PATH=${APPS_ROOT#.}/$PACKAGE
  LATEST_COMMIT_SINCE_LAST_BUILD=$(git log -1 $LAST_COMPLETED_BUILD_SHA..$CIRCLE_SHA1 --format=format:%H --full-diff ${PACKAGE_PATH#/})
...
echo "Changes detected in ${COUNT} apps/ project(s)."

############################################
## 4. CicleCI REST API call
############################################
